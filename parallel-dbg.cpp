...
int main(int argc,char *argv[]){
  ...
  // std::vector<char> buffer(k1+k2);
    ...
    if (nprocs < 2)
    {
      if (rank == 0) { std::cout << "Error! This program requires at least two processes to run. Exiting... \n"; }
      std::exit(1);
    }
    else if (nprocs > 4)
    {
      if (rank == 0) { std::cout << "Expected no more than 4 processes. Exiting... \n"; }
      std::exit(1);
    }
    ...
    { // handler process...
      ...
      // MPI_Recv(buffer.data(), buffer.size(), MPI_PACKED, MPI_ANY_SOURCE, mpiDataTag, MPI_COMM_WORLD, &status);
    
      // MPI_Unpack(buffer.data(), buffer.size(), &position, &workerStartIndex, 1, MPI_INT, MPI_COMM_WORLD);
      // MPI_Unpack(buffer.data(), buffer.size(), &position, workerQVals.data(), dynamicTaskSize, MPI_DOUBLE, MPI_COMM_WORLD);
       ...
       do{
          ...
          MPI_Recv(&avaialbleProcess, 1, MPI_INT, MPI_ANY_SOURCE, 1, MPI_COMM_WORLD, &status);
          if(indexHandled < max_index){
            // send that process the next index to start on in the total list
             MPI_Send(&indexHandled, 1, MPI_INT, avaialbleProcess, 1, MPI_COMM_WORLD);  
             indexHandled += chunkSize;
          }
       ...
       } while(inactiveWorkers < numberOfWorkers); // while some workers are still active
    }
